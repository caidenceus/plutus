...
def fidelity_total_assets(username):
    res = {}
    api = None
    ...
    try:
        for integration in user['integrations']['L']:
            if integration['M']['integration']['S'] == 'fidelity':
                api = Fidelity(
        ...
        res = {
            'error': e,
            'reason': 'Unable to retrieve data from Fidelity'
        }
    finally:
        if api:
            api.teardown()
...
def fidelity_get_account_holdings(username):
    res = {}
    api = None
